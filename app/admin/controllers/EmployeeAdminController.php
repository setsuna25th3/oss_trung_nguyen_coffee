<?php
    ...
    include __DIR__ .'/../admin/models/EmployeeAdmin.php';
    ...
    class EmployeeAdminController {
        ...
        public function addEmployee($employee){
            global $hostname, $username, $password, $dbname, $port;
            $db = new mysqli($hostname, $username, $password, $dbname, $port);
            $sql = "INSERT INTO employee (Name, StoreId, RoleId, Salary) VALUES (?, ?, ?, ?)";
            $stmt = $db->prepare($sql);
        ...
        public function getEmployeeById($employeeId){
            ...
            $db = new mysqli($hostname, $username, $password, $dbname, $port);
            $employee = new EmployeeAdmin();
            $sql = "SELECT * FROM employeeadmin WHERE Id = ?";
            $stmt = $db->prepare($sql);
        ...
        public function updateEmployee($employee){
            global $hostname, $username, $password, $dbname, $port;
            $db = new mysqli($hostname, $username, $password, $dbname, $port);
            $sql = "UPDATE employee SET Name = ?, StoreId = ?, RoleId = ?, Salary = ? WHERE Id = ?";
            $stmt = $db->prepare($sql);
        ...
        public function deleteEmployeeById($employeeId){
            global $hostname, $username, $password, $dbname, $port;
            $db = new mysqli($hostname, $username, $password, $dbname, $port);
            $sql = "DELETE FROM employeeadmin WHERE Id = ?";
            $stmt = $db->prepare($sql);
            $stmt->bind_param("i", $employeeId);
            $isSuccess = $stmt->execute();
            $result = $isSuccess && ($stmt->affected_rows > 0);
